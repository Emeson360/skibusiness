<?php
require_once __DIR__ .'/includes/database.php';

?>

<!DOCTYPE html>

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Subscription</title>
    <link rel="stylesheet" href="assets/css/bootstrap.min.css">
    <link rel="stylesheet" href="assets/css/owl.carousel.min.css">
    <link rel="stylesheet" href="assets/css/owl.carousel.css">
    <link rel="stylesheet" href="assets/css/common.css">
    <link rel="stylesheet" href="assets/css/common-responsive.css">
    <link rel="stylesheet" href="../cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css" integrity="sha512-MV7K8+y+gLIBoVD59lQIYicR65iaqukzvf/nwasF0nqhPay5w/9lJmVM2hMDcnK1OnMGCdVK+iQrJ7lzPJQd1w==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    
    <script src="userbackend/assets/js/jquery-3.6.3.min.js"></script>
    <script src="assets/js/jquery.validate.js"></script>
    <meta name="csrf-token" content="HGYLUX9faW5bU01EPZeCo2eHPa13A7fVaEIFdyvZ" />
    
    
        <link rel="stylesheet" href="assets/css/packages.css">
    <link rel="stylesheet" href="assets/css/packages-responsive.css">
    <script src="../js.paystack.co/v1/inline.js"></script>
</head>

<body>
    <div class="page">
        <header class="main-header">
	<div class="container header-main-box">
		<nav class="navigation">
			<div class="header-content">
				<div class="header-logo-box">
					<a href="index.php">
						<img src="assets/images/maain-logo.png" alt="">
					</a>
					<div class="homeburger-manu" id="header-fa-bar">
						<span></span>
					</div>
				</div>
			</div>

			<div class="nav-manulist-box" >
				<div class="contact-box">
					<div class="nav-numbar">
						<a href="tel: +234 7062993384">
							<svg xmlns="http://www.w3.org/2000/svg" id="Layer_1" x="0" y="0" version="1.1"
								viewBox="0 0 29 29" xml:space="preserve" fill="#F26821" width="28" height="22">
								<path
									d="m20.914 17.743-2.091 1.178a1.319 1.319 0 0 1-1.58-.217l-6.979-6.979a1.32 1.32 0 0 1-.217-1.58l1.178-2.091a1.978 1.978 0 0 0-.325-2.37L7.766 2.55a1.978 1.978 0 0 0-2.798 0L3.545 3.972a5.276 5.276 0 0 0-.793 6.446l.714 1.19a41.36 41.36 0 0 0 14.946 14.631l.141.081c2.102 1.201 4.699.851 6.382-.831l1.486-1.486a1.978 1.978 0 0 0 0-2.798l-3.136-3.136a1.978 1.978 0 0 0-2.371-.326z" />
							</svg>
							+234 7062993384
						</a>
					</div>
					<div class="nav-numbar">
						<a href="mailto:info@EEH.com">
							<svg xmlns="#" viewBox="0 0 128 96" fill="#F26821" width="18"
								height="28">
								<path
									d="M0 11.283V8a8 8 0 0 1 8-8h112a8 8 0 0 1 8 8v3.283l-64 40zm66.12 48.11a4.004 4.004 0 0 1-4.24 0L0 20.717V88a8 8 0 0 0 8 8h112a8 8 0 0 0 8-8V20.717z"
									data-name="Layer 2" />
							</svg>
							enquiries@easternexporthub.com
						</a>
					</div>
				</div>

				<div class="nav-manulist">
					<div class="container">
						<div class="manu-list">
							<ul class="listing-box">
								<li>
									<a href="index.php">Home</a>
								</li>
								<li class="about-main">
									<a href="#">About</a>
									<svg xmlns="#" viewBox="0 0 24 24" width="28"
										fill="#fff">
										<path
											d="M17,9.17a1,1,0,0,0-1.41,0L12,12.71,8.46,9.17a1,1,0,0,0-1.41,0,1,1,0,0,0,0,1.42l4.24,4.24a1,1,0,0,0,1.42,0L17,10.59A1,1,0,0,0,17,9.17Z" />
									</svg>
									<ul class="abourt-list">
										<li>
											<a href="how-E-ExportHub-work.php">
												How E-Eport Hub Works
											</a>
										</li>
										<li>
											<a href="subscription.php">
												Our Plans
											</a>
										</li>
										<li>
											<a href="#">Our Partners</a>
										</li>
									</ul>
								</li>
								<li class="about-main">
									<a href="benefits.php">Benefits</a>
								</li>
																<li class="about-main">
									<a href="#" onclick="event.preventDefault();" >Sign In/Sign Up</a>
									<svg xmlns="#" viewBox="0 0 24 24" width="28"
										fill="#fff">
										<path
											d="M17,9.17a1,1,0,0,0-1.41,0L12,12.71,8.46,9.17a1,1,0,0,0-1.41,0,1,1,0,0,0,0,1.42l4.24,4.24a1,1,0,0,0,1.42,0L17,10.59A1,1,0,0,0,17,9.17Z" />
									</svg>
									<ul class="abourt-list">
										<li data-toggle="modal" data-target="#exampleModal">
											<a href="#" onclick="event.preventDefault();" id="sign_in">
												Sign In
											</a>
										</li>
										<li>
											<a 
												href="index.php#signup">Sign
												Up</a>
										</li>
									</ul>
								</li>
																								<li>	
									<a href="#">Contact</a>
								</li>

							<!--	<span class="manu-btn-box">
									<a href="sponsor.php">Sponsor/Finance A Project >></a>
								</span>-->
							</ul>
						</div>
					</div>
				</div>
			</div>

		</nav>
	</div>
</header>        <main>
    <section class="packages-wrapper-box">
        <div class="container">
            <div class="packages-content-title">
                <h3 class="title-box">
                    WE HAVE A PLAN FOR EVERYONE
                    <div class="head_bg_blue">
                        <div class="head_bg_round"></div>
                    </div>
                </h3>
                <div class="packages-content-btn">
                    <a href="<?php ROOT_URL ?>index.php#signup">Create Free Account</a>
                </div>
            </div>
            <div class="packages-content-box">
                <form method="POST" action="#" accept-charset="UTF-8" class="form-horizontal user-check"
                    role="form">
                                        <input type="hidden" name="amount" value="500000">
                    <input type="hidden" name="currency" value="NGN">
                    <input type="hidden" name="reference" value="T8rsRMHOVdJdPpjuYVDerSKPL">
                    <input type="hidden" name="_token" value="HGYLUX9faW5bU01EPZeCo2eHPa13A7fVaEIFdyvZ">
                    <input type="hidden" name="plan" value="PLN_n1m5etuqo1tciu3">
                    
                    
                    <div class=" packages-content">
                        <div class="packages-title-box">
                           <!-- <h3>BRONZE</h3>-->
                            <h3>ORDINARY MEMBERSHIP</h3>
                        </div>
                        <div class="packages-price">
                            <h4>₦5000</h4>
                        </div>
                        <div class="packages-des">
                            <p>Free Bronze Badge</p>
                            <p>Guaranteed export of One (1) container in a year</p>
                            <p> You get verified buyers from us.</p>
                            <p>Assist in Loan Applications from Financing Partners</p>

                            <p>Special export training on:
                                What to export
                                How to export
                                Where to export
                                Whom to export (off-takers)</p>
                            <p>Enjoy off taking benefit for One (1) commodity</p>
                            <p>Assist with documentations for export</p>
                            <p>Assist with shipping logistics and clearing services</p>
                        </div>
                        <div class="packages-btn-box">
                            <button type="button" value="Pay Now!">
                                <a style="border: none;" href="<?php ROOT_URL ?>index.php#signup">Subscribe</a> 
                            </button>
                        </div>
                    </div>
                </form>

                <form method="POST" action="#" accept-charset="UTF-8" class="form-horizontal user-check"
                    role="form">
                                        <input type="hidden" name="amount" value="2000000">
                    <input type="hidden" name="currency" value="NGN">
                    <input type="hidden" name="reference" value="1Ocqq2fOnYdKcKc7Nc60wNnZr">
                    <input type="hidden" name="_token" value="HGYLUX9faW5bU01EPZeCo2eHPa13A7fVaEIFdyvZ">
                    <input type="hidden" name="plan" value="PLN_51dwxj8e427a3gf">
                    <div class=" packages-content">
                        <div class="packages-title-box">
                            <h3>ORDINARY MEMBERSHIP</h3>
                        </div>
                        <div class="packages-price">
                            <h4>₦20000</h4>
                        </div>
                        <div class="packages-des">
                            <p>Free Silver Badge</p>
                            <p>Guaranteed export of Three (3) containers in a year</p>
                            <p>You get verified buyers from us</p>
                            <p>Assist in Loan Applications from Financing Partners</p>
                            <p>Special export training on:
                                What to export
                                How to export
                                Where to export
                                Whom to export (off-takers)</p>
                            <p>Enjoy off taking benefits for up to Two (2) different commodities</p>
                            <p>Assist with documentations for export</p>
                            <p>Assist with shipping logistics and clearing services</p>

                        </div>
                        <div class="packages-btn-box">
                            <button type="button" value="Pay Now!">
                                <a style="border: none;" href="<?php ROOT_URL ?>index.php#signup">Subscribe</a> 
                            </button>
                        </div>
                    </div>
                </form>
                <form method="POST" action="#" accept-charset="UTF-8" class="form-horizontal user-check"
                    role="form">
                                        <input type="hidden" name="amount" value="5000000">
                    <input type="hidden" name="currency" value="NGN">
                    <input type="hidden" name="reference" value="FC5NKRllzrqP9a0DUvxoDQJV2">
                    <input type="hidden" name="_token" value="HGYLUX9faW5bU01EPZeCo2eHPa13A7fVaEIFdyvZ">
                    <input type="hidden" name="plan" value="PLN_1k2xjsz90st9b1n">
                    <div class=" packages-content">
                        <div class="packages-title-box">
                            <h3>ORDINARY MEMBERSHIP</h3>
                        </div>
                        <div class="packages-price">
                            <h4>₦50000</h4>
                        </div>
                        <div class="packages-des">
                            <p>Free Gold Badge</p>
                            <p>Guaranteed export of Five (5) containers in a year</p>
                            <p>You get verified buyers from us.</p>
                            <p>Assist in Loan Applications from Financing Partners</p>
                            <p>Special export training on:
                                What to export
                                How to export
                                Where to export
                                Whom to export (off-takers)</p>
                            <p>Enjoy off taking benefits for up to Three (3) different commodities</p>
                            <p>Assist with documentations for export</p>
                            <p>Assist with shipping logistics and clearing services</p>
                        </div>
                        <div class="packages-btn-box">
                            <button type="button" value="Pay Now!">
                                <a style="border: none;" style="border: none;" href="<?php ROOT_URL ?>index.php#signup">Subscribe</a> 
                            </button>
                        </div>
                    </div>
                </form>
                <form method="POST" action="#" accept-charset="UTF-8"
                    class="form-horizontal user-check" role="form">
                                        <input type="hidden" name="amount" value="5000000">
                    <input type="hidden" name="currency" value="NGN">
                    <input type="hidden" name="reference" value="ZGWSRmnhAtTdd0TxbfxAKhvBX">
                    <input type="hidden" name="_token" value="HGYLUX9faW5bU01EPZeCo2eHPa13A7fVaEIFdyvZ">
                    <input type="hidden" name="plan" value="PLN_2s5790t19rslqhg">
                    <div class=" packages-content">
                        <div class="packages-title-box">
                            <h3>SUPER EXCLUSIVE</h3>
                        </div>
                        <div class="packages-price">
                            <h4>₦500000</h4>
                        </div>
                        <div class="packages-des">
                            <p>Free Super Exclusive Badge</p>
                            <p>Guaranteed export of Ten (10) containers in a year</p>
                            <p>You get verified and genuine buyers from us</p>
                            <p>Assist in Loan Applications from Financing Partners</p>
                            <p>Provide with extensive international B2B engagements</p>
                            <p>Special export training on:
                                What to export
                                How to export
                                Where to export
                                Whom to export (off-takers)</p>
                            <p>Enjoy off taking benefits for up to five (5) different commodities</p>
                            <p>Assist with documentations for export</p>
                            <p>Assist with first rate shipping logistics and clearing services</p>
                            <p>Provision of warehousing services for a variety of commodities locally.</p>
                          <!--  <p>We provide warehousing services for a variety of commodities in China.</p>-->
                            <p>We provide consultancy services (Market intelligence, Product specification, Packaging,
                                certifications & Value addition) for you where applicable.</p>
                            <p>We advertise your product on our social media Platform</p>
                            <p>Assist with language translator service</p>
                            <p>We provide Visa assistance services</p>
                        </div>
                        <div class="packages-btn-box">
                            <button type="button" value="Pay Now!">
                                <a style="border: none;" style="border: none;" href="<?php ROOT_URL ?>index.php#signup">Subscribe</a> 
                            </button>
                        </div>
                    </div>
                </form>
                <form method="POST" action="#" accept-charset="UTF-8"
                    class="form-horizontal user-check" role="form">
                                        <input type="hidden" name="amount" value="5000000">
                    <input type="hidden" name="currency" value="NGN">
                    <input type="hidden" name="reference" value="ukmKkPWDWEgsCwBNDo2qWSUOk">
                    <input type="hidden" name="_token" value="HGYLUX9faW5bU01EPZeCo2eHPa13A7fVaEIFdyvZ">
                    <input type="hidden" name="plan" value="PLN_osueiosjmanwgkt">
                    <div class=" packages-content">
                        <div class="packages-title-box">
                            <h3>PLATINUM ELITE</h3>
                        </div>
                        <div class="packages-price">
                            <h4>₦5000000</h4>
                        </div>
                        <div class="packages-des">
                            <p>Free Platinum Elite Badge</p>
                            <p>Guaranteed export of Twenty (20) containers in a year</p>
                            <p>You get verified and genuine buyers from us</p>
                            <p>Assist in Loan Applications from Financing Partners</p>
                            <p>Provide with extensive international B2B engagements</p>
                            <p> Special export training on:
                                What to export
                                How to export
                                Where to export
                                Whom to export (off-takers)</p>
                            <p>Enjoy off taking benefits for up to Ten (10) different commodities</p>
                            <p>Assist with documentations for export</p>
                            <p>Assist with first rate shipping logistics and clearing services</p>
                            <p>Provision of warehousing services for a variety of commodities locally.</p>
                           <!-- <p>We provide warehousing services for a variety of commodities in China.</p>-->
                            <p>We provide consultancy services (Market intelligence, Product specification, Packaging,
                                certifications & Value addition) for you where applicable.</p>
                            <p>We advertise your product on our social media Platform</p>
                            <p>Assist with language translator service</p>
                            <p>We provide Visa assistance services</p>
                        </div>
                        <div class="packages-btn-box">
                            <button type="button" value="Pay Now!">
                                <a style="border: none;" href="<?php ROOT_URL ?>index.php#signup">Subscribe</a> 
                            </button>
                        </div>
                    </div>
                </form>
                
            </div>
        </div>
    </section>
     <!-- <section class="latest-news-wrapper ">
            <div class="container">
                <div class="latest-news-content">
                    <div class="latest-title-box">
                        <h3 class="title-box">
                            LATEST NEWS
                            <div class="head_bg_blue">
                                <div class="head_bg_round"></div>
                            </div>
                        </h3>
                    </div>
                    <div class="latest-slider-box">
                        <div class="owl-slider">
                            <div id="latest-slider" class="owl-carousel">
                                <div class="item">
                                    <div class="latest-slider-content-box">
                                        <div class="col-lg-6 latest-card-box">
                                            <div class="latest-title-box">
                                                <h4 class="title-box" style="font-size:16px;">
                                                    EXPORTABLE AGRO COMMODITIES
                                                </h4>
                                                <div class="exportable d-flex"
                                                    style="align-items:baseline;justify-content: space-between">
                                                    <ul>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Cocoa
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Sesame Seeds
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Ginger
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Cashew Nuts
                                                        </li>

                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Beans
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Soya Beans
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Millet
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Tiger Nuts
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Garlic
                                                        </li>
                                                    </ul>
                                                    <ul>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Sorghum
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Morning Seeds
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Shear Butter
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Hibiscus Flower
                                                        </li>

                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Hide and Skin
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Rubber
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Cassia Tora Seeds
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Palm Nut Kernel
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Gum Arabic
                                                        </li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-lg-6 latest-card-box">
                                            <div class="latest-title-box">
                                                <h4 class="title-box" style="font-size:16px;">
                                                    EXPORTABLE MINERALS
                                                </h4>
                                                <div class="exportable d-flex"
                                                    style="align-items:baseline;justify-content: space-between">
                                                    <ul>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Iron Ore
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Barite Ore
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Lithium
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Copper Ore
                                                        </li>

                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Lead Ore
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Tin Ore
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Silver
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Gold
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Precious Metal
                                                        </li>
                                                    </ul>
                                                    <ul>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Tourmaline
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Sapphire
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Emerald
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Ruby
                                                        </li>

                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Topaz
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Zircon
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Garnet
                                                        </li>
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="item">
                                    <div class="latest-slider-content-box">
                                        <div class="col-lg-6 latest-card-box">
                                            <div class="latest-title-box">
                                                <h4 class="title-box" style="font-size:16px;">
                                                    EXPORTABLE COUNTRIES
                                                </h4>
                                                <div class="exportable d-flex"
                                                    style="align-items:baseline;justify-content: space-between">
                                                    <ul>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            China
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Hong Kong
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Macau
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Japan
                                                        </li>

                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            North Korea
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            South Korea
                                                        </li>
                                                    </ul>
                                                    <ul>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Indonesia
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Myanmar
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Mongolia
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Philippines
                                                        </li>

                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            Cambodia
                                                        </li>
                                                        <li>
                                                            <div class="tabs-arrow">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="#fff"
                                                                    width="32" height="32" viewBox="0 0 64 64">
                                                                    <path
                                                                        d="M13.707 32.293l-1.414 1.561 12 12.146h1.414l24-24.146-1.414-1.488L25 43.623z">
                                                                    </path>
                                                                </svg>
                                                            </div>
                                                            East Timor
                                                        </li>
                                                        
                                                    </ul>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </section> -->
    <div class="modal fade" id="error-modal" tabindex="-1" role="dialog" aria-labelledby="error-modal"
        aria-hidden="true">
        <div class="modal-dialog dialog-popup modal-dialog-centered" role="document">
            <div class="modal-content modal-popup">
                <div class="modal-header header-popup">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="login_message" style="color: #fff;">
                        
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>

    <footer class="main-footer">
   <!-- <div class="container">
        <div class="footer content-box">
            <div id="footer">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="footer-img">
                                <a href="index.php">
                                    <h1>NEXHub</h1>
                                </a>
                            </div>
                            <p>NEXHub is a platform for different individuals set to meet the purpose of
                                international trade, it has a dynamic electronic and digital platform that
                                connects buyers and sellers who are exporters and importers or domestic vendors
                                along with other value-chain stakeholders, enabling them to carry out
                                transactions in a safe and coordinated manner by the operators of the platform.
                            </p>
                        </div>-->

                       <!-- <div class="col-lg-4">
                            <h3>Benefits <div class="head_bg_blue head_bg_blue1">
                                    <div class="head_bg_round head_bg_round1"></div>
                                </div>
                            </h3>

                            <div class="links_warp">

                                <li><a href="benefits.php">Learn how to export</a></li>
                                <li><a href="benefits.php">Learn what to export</a></li>
                                <li><a href="benefits.php">Learn where to export</a></li>
                                <li><a href="benefits.php">Learn to whom to export</a></li>
                                <li><a href="benefits.php">Get verified off-takers</a></li>-->
                                <!-- <li><a href="benefits">Business Support Benefit</a></li>
                                <li><a href="benefits">Vehicle Trade-in Benefit</a></li>
                                <li><a href="benefits">Education Support Benefit</a></li>
                                <li><a href="benefits">Property Ownership Benefit</a></li>
                                <li><a href="benefits">Organization Support Benefit</a></li> -->
                            </div>
                            <!-- links_warp -->

                        </div>
                        <!-- col-lg-4 -->

                       <!-- <div class="col-lg-4">
                            <h3>get in touch <div class="head_bg_blue head_bg_blue1">
                                    <div class="head_bg_round head_bg_round1"></div>
                                </div>
                            </h3>
                            <div class="links_warp">

                                <p><strong>Corporate Headquarters:</strong><br>
                                    Plot 252, Cadastral Zone C16, Idu Industrial Area 1, Idu, Abuja.</p>

                                <p><strong>Hotlines:</strong><br>
                                    <a href="#"> 07000933622</a> <br>
                                    <a href="#"> 096232996</a> <br>
                                    <a href="#"> 09122223978</a> <br>
                                <p>
                                    <strong>Email:</strong><br>
                                <div class="footer-emil-box">

                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 96" fill="#fff"
                                        width="18" height="28">
                                        <path
                                            d="M0 11.283V8a8 8 0 0 1 8-8h112a8 8 0 0 1 8 8v3.283l-64 40zm66.12 48.11a4.004 4.004 0 0 1-4.24 0L0 20.717V88a8 8 0 0 0 8 8h112a8 8 0 0 0 8-8V20.717z"
                                            data-name="Layer 2"></path>
                                    </svg>
                                    <span> enquiries@nigerianexportershub.com </span>
                                </div>

                                </p>-->

                                <!-- <div class="media_ico">
                                    <a class="fb" href="#"><i class="fa fa-facebook"></i></a>
                                    <a class="bird" href="#"><i class="fa fa-twitter"></i></a>
                                    <a class="link" href="#"><i class="fa fa-linkedin"></i></a>
                                    <a class="g_plus" href="#"><i class="fa fa-google-plus"></i></a>
                                </div> -->

                          <!--  </div>
                        </div>

                    </div>
                </div>
            </div>

        </div>
    </div>-->
    <div class="footer-last-box">
        <div class="container">
            <div class="footer-sub-sec">
                <div class="footer-span">
                    <span>
                        ©2023 Eastern Exporters Hub. All Rights Reserved.
                    </span>
                </div>
                <div class="footer-terms">
                    <p class="second"><a href="terms.php">Terms</a> | <a href="privacy.php">Privacy</a> Powered by - <a
                            href="#" target="_blank">Skibusiness</a>
                    </p>
                </div>
            </div>
        </div>
    </div>
    <div class="" id="loader" onclick="$(this).hide()" style="display:none;">
        <div class="modal-backdrop-loader" >
            <i class="fa fa-spinner fa-spin text-white font-30"
                style="position: absolute;right: 50%;top: 50%;font-size: 26px;"></i></div>
    </div>
</footer>
        <!-- popup manu -->
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog dialog-popup modal-dialog-centered" role="document">
        <div class="modal-content modal-popup">
            <div class="modal-header header-popup">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class='sing-in-popup'>
                    <div class="login_message"></div>
                    <form action='<?= ROOT_URL ?>ski/signin-logic.php' method='POST'>
                        <!-- <input type="hidden" name="_token" value="HGYLUX9faW5bU01EPZeCo2eHPa13A7fVaEIFdyvZ">                         -->
                        <div class='sing-in-box'>
                            <div class='popup-input'><label for=''>Email</label><input type='text'
                                    name='email' placeholder='Email' value=''>
                                                            </div>
                            <div class='popup-input'><label for=''>Password</label><input type='password'
                                    name='password' placeholder='Password' value=''>
                                                            </div>
                            <div class='popup-btn'><button type='submit' name="signin">Login</button></div>
                        </div>
                    </div>
                    <div class="text-center mt-3" >
                        <a href="forgot-password.php" class="forgotpasswordbtn">
                            Forgot password?
                        </a>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
    </div>
    <script src="assets/js/bootstrap.bundle.min.js"></script>
    <script src="assets/js/owl.carousel.js"></script>
    <script src="assets/js/custom.js"></script>
    
    
</body>


</html>
